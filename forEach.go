...
// ForEach returns a Pipeable that applies a function to each item from the input stream.
// It is intended for side effect and the output stream will only emit the errors returned by the function.
func ForEach[T any](f func(context.Context, Item[T]) error, opt ...Option) Pipeable[T, struct{}] {
