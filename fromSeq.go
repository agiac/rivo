...
func FromSeq[T any](seq iter.Seq[T]) Generator[T] {
	return func(ctx context.Context, in <-chan None, errs chan<- error) <-chan T {
		out := make(chan T)
...
func FromSeq2[T, U any](seq iter.Seq2[T, U]) Generator[FromSeq2Value[T, U]] {
	return func(ctx context.Context, in <-chan None, errs chan<- error) <-chan FromSeq2Value[T, U] {
